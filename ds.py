...
def train(model,data, epochs, sources):
    estart = time.time()
    total_loss = 0
    num = 0
    for ep in range(epochs):
        current_time = time.time()
        for idx, (i,t ,song_name) in enumerate(data):
            ...
            if (idx+1) % 100 == 0:
                ...
                torch.save(model.state_dict(), f'ds_{sources}.pth')
                current_time = time.time()
...
num_layers = 6
num_heads = 3
dropout = 0.5 # 0.1 = 라이드, 0.5 = 심벌
hidden_dim = 1024
device = "mps" if torch.backends.mps.is_available() and torch.backends.mps.is_built() else 'cpu'
drum_path = '/Volumes/VIDEO/DrumSeparateSource/'

for source in ['kick','snare','hihat','crash','tom','ride']:
    print(source, ' Start')
    ...
    optimizer = optim.Adam(params=model.parameters(), lr=0.0001) # 0.001 = 라이드가 좋음 0.0001 = 심벌이 좋음
    train(model, train_loader, 3, source)
